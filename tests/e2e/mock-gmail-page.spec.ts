...
async function setupMockPage(page: Page): Promise<void> {
  ...
  await page.waitForFunction(() => {
    const w = window as unknown as { __gisoShowOverlay?: () => void };
...
test.describe("Mock Gmail workflow", () => {
  test("overlay opens, shows brand credit, analyzes, and finds groups", async ({ page }) => {
    ...
    await page.evaluate(() => {
      (window as unknown as { __gisoShowOverlay: () => void }).__gisoShowOverlay();
    });
