...
import { scoreCandidate, selectUnambiguous, type ScoredCandidate } from "@/gmail/candidate-scoring";
...
describe("selectUnambiguous", () => {
  ...
  it("rejects a candidate below the minimum score", () => {
    const a = document.createElement("button");
    const candidates: ScoredCandidate<HTMLElement>[] = [{ element: a, score: 89, evidence: [] }];
    expect(selectUnambiguous(candidates, 90, 20)).toBeNull();
  });
});
