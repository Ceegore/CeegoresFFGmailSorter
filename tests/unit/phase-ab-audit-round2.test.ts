...
describe("ROUND2: no auto-click path reachable in safe mode", () => {
  it("SELECTING_PAGE state is unreachable via normal flow in safe mode", () => {
    // In safe mode, SEARCH_READY dispatches SEARCH_READY_MANUAL, not the auto path.
    // So SELECTING_PAGE should never be entered from the controller.
    // Verify the state machine still allows it (for when SAFE_MODE is off),
    // but the controller path to it is gated.
    const s = reduceAppState(
      { ...initialState, workflow: "WAITING_SEARCH_RESULTS" as const },
      { type: "SEARCH_READY" },
    );
    expect(s.workflow).toBe("SELECTING_PAGE"); // state machine itself is fine
  });
});
