...
import type { AnalysisResult, AppState, SenderGroup } from "@/shared/types";

function makeController() {
  return createAppController(createStore(initialState, reduceAppState));
}

function render(state: AppState): ShadowRoot {
...
describe("views: RESULTS_READY", () => {
  ...
  it("filter input wires to setFilter on the controller", () => {
    ...
    const input = shadow.querySelector<HTMLInputElement>('[data-testid="giso-filter"]');
  ...
  it("sort select wires to setSort", () => {
    ...
    const select = shadow.querySelector<HTMLSelectElement>('[data-testid="giso-sort"]');
  ...
  it("find-all button wires to selectGroup", () => {
    ...
    const findAll = shadow.querySelector<HTMLElement>('[data-testid="giso-find-all"]');
...
function makeAppControllerWithSpy(method: "selectGroup" | "ignoreGroup") {
  const store = createStore(initialState, reduceAppState);
  const real = createAppController(store);
  const spy: Partial<ReturnType<typeof createAppController>> = { ...real, [method]: vi.fn() };
  return spy as ReturnType<typeof createAppController>;
}
